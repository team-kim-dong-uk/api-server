...
public class PhotoService {
   ...
   */
  public PhotoDetailDto getPhotoDetail(String userId, String photoId)
  ...
  public List<PhotoOutlineDto> findPhotos(List<String> tags, String sortBy, String uploaderId,
      String findAfterId, int fetchSize) {
    List<Photo> photos;

    /* 1. create query parameter */
    ObjectId uploaderObjectId =
        StringUtils.isNullOrEmpty(uploaderId) || !ObjectId.isValid(uploaderId)
            ? null
            : new ObjectId(uploaderId);
    Sort sort = StringUtils.isNullOrEmpty(sortBy)
        ? Sort.by(Photo.DEFAULT_SORT)
        : Sort.by(sortBy);
    ObjectId findAfterObjectId =
        StringUtils.isNullOrEmpty(findAfterId) || !ObjectId.isValid(findAfterId)
    ...
    if (uploaderObjectId == null) {
      ...
      photos = (tags.size() == 0)
          ? photoRepository
          .findAllByUploaderIdAndIdAfter(uploaderObjectId, findAfterObjectId, pageable)
          : photoRepository
              .findAllByUploaderIdAndTagsInAndIdAfter(uploaderObjectId, tags, findAfterObjectId,
                  pageable);
    }
  ...
  public List<PhotoOutlineDto> findPhotosUploadedBy(String userId, String findAfterId,
      int fetchSize) {
