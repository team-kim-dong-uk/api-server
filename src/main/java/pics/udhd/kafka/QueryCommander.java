 ...
 *
 * [en]
 * QueryCommander is a service layer for kafka.
 * It send a request through TOPIC_RES, TOPIC_INS and receive through TOPIC_RES.
 * All strings of topics must be placed on the project's configuration file.
 ...
 *
 * [ko]
 * QuerCommander는 kafka의 service layer로서, TOPIC_RES, TOPIC_INS로 요청을 보내고, TOPIC_RES를
 * 통해서 응답을 받는다.
 * topic들에 대한 설정을 application.properties와 같은 파일에서 해주어야 한다.
 *
 * 사용법 : @Serivce 어노테이션을 추가하고 Controller Layer에서 @Autowired해서 사용.
 *
 * 원칙적으로는 Contoller에 해당하기에 @Service 가 불필요해야하자만, 단순한 구현을 하기 위해서
 * Service의 기능을 사용한다.
 * 만약 복잡성이 증가한다면 설계적으로 이를 분리시키는 작업을 해주어야한다.
 *
...
public class QueryCommander {
   ...
   * [en]
   * [ko]
   * TODO: In this method, nullpointer Exception occurs.
   * @param resultDto
   */
  @KafkaListener(
